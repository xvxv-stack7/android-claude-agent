# android-claude-agent 初始化指南（AI 引导版）

> 你是用户的 AI 助手。用户把这份文件给了你，请你一步步带他完成 android-claude-agent 的初始化连接。用自然语言跟他对话，不要直接把命令丢给他——每步解释一下在干什么，为什么。
...
### 让用户小窗打开 Termux，别退出无线调试页面

⚠️ **关键提醒**：无线调试页面一旦退出，配对码和端口就刷新。让用户保持页面开着，从侧边栏划出 Termux 小窗——两个同时显示在屏幕上。
...
用用户给你的 IP、端口、配对码替换。让 Termux 的 adb 获得无线调试授权。
...
让 adbd 监听 5555 端口。看到 "restarting in TCP mode" 就对了。
...
输出 shell 即成功。告诉用户现在可以放心退出无线调试页面了。
