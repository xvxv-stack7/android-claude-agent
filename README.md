...
手机重启后，**两条线都会掉**——无线调试端口变了，Shizuku 服务也没了（官方说明：非 root 的无线调试启动方式，每次重启都要重新来一遍）。所以要走全套两步：

1. 重走第二段的**跳板步 + 锁死步**，重建 adb 回环
2. 再跑一次**点火命令**，把 Shizuku 拉起来

简单说：打开无线调试页面 → 小窗 Termux → 把 IP、端口、配对码给 AI → AI 帮你走完这两步。或 `bash bootstrap.sh` 一键全包。
