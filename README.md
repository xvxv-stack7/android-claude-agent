...
装完之后它不是"你问一句它答一句"——它自己看得见，也自己会判断：
...
- **感知**：屏幕 / 前台 App / 电量 / 步数 / 短信 / 日历 / 耳机 / 正在听什么
- **叫醒**：有事立刻叫它；没事也定时把它叫起来，回头看一眼
- **判断**：说不说、做什么 —— 这一步归 Claude，不写死规则
- **动作**：弹一句 / 发通知 / 停应用 / 放首歌 / 点屏幕

**这四样就是"主动性"本身。** 剩下的一切——电量、短信、日历、音乐、防沉迷——都只是挂在这套骨架上的具体能力。换一个传感器就换一件事，骨架不动。

### 一条事件长什么样

传感器读到的东西，最后都汇成一条事件，落进同一个文件：

```json
{"event": "left_chat", "fg_app": "com.openai.chatgpt", "battery": "73", "screen": "Awake", "ts": 1790017624, "consumed": true}
```

`event` 是发生了什么，`ts` 是什么时候，中间三个是**当时的现场**——你在哪个 App、多少电、屏亮没亮。Claude 就是攥着这些做判断的。

`consumed` 是"读过了"的标记：同一条事件不会被反复喂给 AI，免得同一件事在你耳边叨叨两遍。

这行小标记背后是一条设计原则：

> **主动性不是"让 AI 能调更多工具"，而是"只在值得的时候把它叫醒，每次只让它看到真正该看的那点现实"。**

叫得越少，每一声才越值钱。

### 为什么非得是"触发型"

主动型 AI 最贵的地方，是**"一直醒着"**这件事。

让它常驻、每隔半分钟问一遍"现在要不要说话"——最直接，也最烧 token：绝大多数时候，它只是在确认"没事"。

这套走的是另一条路：**没事的时候让它睡着，睡着不花钱。** 有事才叫醒，醒了只看一件事，看完接着睡。

省下来的是真金白银，"不烦人"是顺带的。

**状态和事件分成两个文件**：一个记"你此刻在哪儿"（持续更新），一个记"刚发生了什么"（一次性）。混在一块就乱。

装上之后，你的 Claude 能读你的屏幕、感知你的状态、替你操作手机，也会在深夜屏幕还亮着的时候自己出声。无论你是想让 AI 当编程搭子，还是想跟 AI 谈一场摸得到手机的恋爱，这里都是起点。
...
## 🧵 底下是怎么连的：adb 线 / rish 线

Claude 要动手，得有路通到 Android。路有两条，并行走：
...
| **music-control** | `skills/music-control/` | 🎵 旧版（网易云API+mpv）→ **新版见 [android-claude-music](https://gitee.com/xvxv663/android-claude-music)** |
...
装到这儿，你的 AI 已经住进手机、也摸得到手机了 —— 但它还听不见你耳机里的声音。

→ **[android-claude-music](https://gitee.com/xvxv663/android-claude-music)**

它听得见你在听什么、跟得上唱到哪一句；在你没叫它的时候，挑一句有意思的递到你眼前。
还能自己挑时候给你放一首 —— 放歌走虚拟副屏，**全程不占你的屏**。

**三部曲：住进来（wechat）→ 摸得到（agent）→ 一起听（music）。**
...
- [ ] **语音感知**：Whisper.cpp 本地语音识别，让 AI "听到"你说话
