...
| 线路 | 命令 | 性情 | 适合 |
|---|---|---|---|
| ② **adb 线** | `adb -s 127.0.0.1:5555 shell 命令` | **快**：直连 adbd，一步到位 | 日常高频、轮询、监控 |
| ① **rish 线** | `rish -c '命令'` | **慢一档**：经 Shizuku 中转；服务偶尔会被系统杀，断了要重新拉起 | 偶尔调用、临时顶班 |
...
手机重启 → 无线调试端口变了 → 重走第二段即可（Shizuku 通常还在）。简单说：打开无线调试页面 → 小窗 Termux → 把 IP、端口、配对码给 AI → AI 帮你连回来。或 `bash bootstrap.sh` 自动处理。
